...
let introStep = 0; 
const SERVER_URL = "http://127.0.0.1:8000"; // [중요] 서버 주소
...
function runIntroSequence() {
    ...
    if (introStep === 0) {
        typeWriter(dialogueEl, "가여운 영혼이여... 죽음을 맞이했는가?\n내가 너에게 새로운 삶을 부여하겠다.", () => {
            nextBtn.classList.remove('hidden');
            nextBtn.onclick = () => { introStep++; runIntroSequence(); };
        });
    } else if (introStep === 1) {
        nextBtn.classList.add('hidden');
        typeWriter(dialogueEl, "새로운 세계에서 불릴 그대의 이름은 무엇인가?", () => {
            inputArea.classList.remove('hidden');
            inputArea.innerHTML = `<input type="text" id="intro-name-input" class="intro-input" placeholder="이름을 입력하세요"> <button id="submit-name" class="menu-btn" style="padding:5px 10px;">확정</button>`;
            document.getElementById('submit-name').onclick = () => {
                ...
                introStep++; runIntroSequence();
            };
        });
    } else if (introStep === 2) {
        inputArea.classList.add('hidden');
        typeWriter(dialogueEl, `${playerCreationData.name}... 좋은 울림이구나.\n그대가 가졌던 잠재력은 어느 정도였지?`, () => {
            inputArea.classList.remove('hidden');
            inputArea.innerHTML = `
                <div class="stat-distribute-row"><label>용기 (HP/물방)</label><input type="number" class="intro-input" id="s-courage" value="5" min="1" max="10" style="width:60px;"></div>
                <div class="stat-distribute-row"><label>지혜 (MP/마방)</label><input type="number" class="intro-input" id="s-wisdom" value="5" min="1" max="10" style="width:60px;"></div>
                <div class="stat-distribute-row"><label>절제 (피해감소)</label><input type="number" class="intro-input" id="s-temperance" value="5" min="1" max="10" style="width:60px;"></div>
                <div class="stat-distribute-row"><label>정의 (치명/회피)</label><input type="number" class="intro-input" id="s-justice" value="5" min="1" max="10" style="width:60px;"></div>
                <button id="submit-stats" class="menu-btn" style="margin-top:10px; width:100%;">이대로 다시 태어나기</button>
            `;
            document.getElementById('submit-stats').onclick = () => {
        ...
        inputArea.classList.add('hidden');
        typeWriter(dialogueEl, "준비는 끝났다. 심연이 그대를 기다린다.\n행운을 비네.", () => {
...
function setupGameButtons() {
    document.getElementById('btn-attack').addEventListener('click', () => combatTurn('attack'));
    document.getElementById('btn-defend').addEventListener('click', () => addLog("🛡️ 방어 태세를 취합니다."));
    document.getElementById('btn-run').addEventListener('click', () => addLog("🏃 도망칠 수 없습니다!"));
...
async function combatTurn(action, customInput = null) {
    if (gameState.monsters.length === 0) {
        if(action === 'custom') addLog("반응할 대상이 없습니다.");
        else addLog("공격할 대상이 없습니다.");
        return;
    }
    
    let targetIdx = gameState.monsters.findIndex(m => m.hp > 0);
    if (targetIdx === -1) {
        addLog("모든 적을 이미 물리쳤습니다.");
        return;
    }
    ...
    try {
        ...
        loadingDiv.innerText = "... AI가 상황을 묘사하는 중 ...";
        loadingDiv.style.color = "#888";
        loadingDiv.id = "ai-loading";
        document.getElementById('log-window').appendChild(loadingDiv);
        ...
        // AI가 써준 로그 출력
        result.logs.forEach(txt => addLog(txt, "log-highlight"));
        
        updateDisplayStats();
        renderMonsters();
        
        if (result.status === 'victory') {
            document.getElementById('main-screen').innerHTML = `<div style="font-size:20px; color:#f1c40f; margin-top:50px;">VICTORY</div>`;
            gameState.monsters = []; 
        } else if (result.status === 'defeat') {
            alert("당신은 쓰러졌습니다... Game Over");
            location.reload();
        }
    } catch (err) {
        console.error(err);
        addLog("[오류] 서버 통신 실패 (터미널을 확인하세요)");
    }
}
...
async function enterLocation(locId) {
    try {
        ...
        addLog(`[시스템] ${data.location}에 진입했습니다.`);
        document.getElementById('story-text').innerHTML = data.description + "<br><br>무엇을 하시겠습니까?";
        
        if (data.monsters && data.monsters.length > 0) {
            gameState.monsters = data.monsters;
            addLog(`⚠️ [${data.group_info.name}]이(가) 나타났습니다!`, "log-highlight");
            renderMonsters(); 
        } else {
            gameState.monsters = [];
            document.getElementById('main-screen').innerHTML = `<div style="color:#888; margin-top:50px;">안전해 보입니다.</div>`;
        }
    } catch (err) {
        console.error(err);
        addLog("[오류] 서버 연결 실패");
